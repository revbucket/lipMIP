...
def main():
	...
	LAYER_SEQ = [([2] + [20 for _ in range(i)] + [2]) for i in range(1, 15)]
	def NAME_FXN(network):
		""" Returns a string based on the network """
		depth = network.num_relus

		return '%s_DEPTH%04d' % (NAME, depth)
	# ================================================================
	...
	l2_penalty = train.LpWeightReg(scalar=1e-4, lp='l1')
	...
	for layer_size in LAYER_SEQ:
		print("-" * 30, "Starting training:", layer_size)
