...
def build_forward_pass_constraints(relunet, gurobi_squire):
	...
	if isinstance(relunet.fcs[-1], nn.Linear):
		...
		add_linear_layer_mip(relunet, len(relunet.fcs) - 1, 
...
def add_backprop_linear_layer(network, layer_no, squire,
							  input_key, output_key):
...
def add_abs_layer(network, squire, hyperbox_bounds, 
...
def set_l1_objective(squire, abs_key):
	...
	squire.model.setObjective(sum(abs_vars), gb.GRB.MAXIMIZE)
	squire.update()
